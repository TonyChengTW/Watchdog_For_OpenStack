...
mongodb_opts = [
    cfg.StrOpt('host', default='127.0.0.1',
               help='The mongodb bind address'),
    cfg.IntOpt('port', default=11211,
               help='The mongodb port where listen on'),
]

CONF = cfg.CONF
CONF.register_opts(mongodb_opts, group='mongodb')
...
class Mongodb(monitor_base.MonitorBase):

    def __init__(self):
        super(Mongodb, self).__init__()
        self.name = 'mongod'
        self.description = "Mongodb service"
        self.service_name = 'mongodb'

        self.host = CONF.mongodb.host
        self.port = CONF.mongodb.port
    ...
    def is_zombie(self):
        cmd = "mongo -p %s --eval 'db.stats().ok' --quiet %s" \
              % (self.port, self.host)
        result, stdout, stderr = self.run_cmd(cmd)
        if result == 0 and stdout[0] == '1':
            return False
    ...
    def check_all(self):
        cmd_list = [
            {'handler': 'socket_status',
             'success': ['mongodb_cluster success',
                         'The socket checking is success'],
             'failure': ['mongodb_cluster failure',
    ...
    def process_status(self):
        cmd = "ps -ef | grep /usr/bin/mongod | grep -vq grep"
        result, stdout, stderr = self.run_cmd(cmd)
        return result
    ...
    def replica_status(self):
        cmd = "mongo -p %s --eval 'rs.status().ok' --quiet %s" \
              % (self.port, self.host)
        result, stdout, stderr = self.run_cmd(cmd)
        if result == 0 and stdout[0] == '1':
            return 0
