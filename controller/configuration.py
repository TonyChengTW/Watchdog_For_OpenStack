...
class Configuration(object):
    ...
    def __init__(self, monitor_opts, config_group=None):
        ...
        if self.config_group:
            self._ensure_config_values(monitor_opts)
            self.local_conf = CONF._get(self.config_group)
        else:
            self.local_conf = CONF

    def _ensure_config_values(self, monitor_opts):
        CONF.register_opts(monitor_opts, group=self.config_group)

    def append_config_values(self, monitor_opts):
        self._ensure_config_values(monitor_opts)
